...
struct NodeHash
{
   unsigned long operator() (const Node & node) const
      {
         unsigned long l = (unsigned long) node.left;
         unsigned long r = (unsigned long) node.right;
         return l + (r >> 2) + (r << 3);
      }
};

static std::hash_set <Node, NodeHash> CanonicalNodeSet;
