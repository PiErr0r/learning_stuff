  ...
  if (has_userns) {
    ...
    for (char **file = (char *[]) { "uid_map", "gid_map", 0 }; *file; file++) {
      if (snprintf(path, sizeof(path), "/proc/%d/%s", child_pid, *file)
          > sizeof(path)) {
        fprintf(stderr, "snprint too big? %m\n");
        return -1;
      }
...
int choose_hostname(char *buff, size_t len)
{
  ...
  static const char *minor[] = { "ace", "two", "three", "four", "five", "six", "seven", "eight",
                                 "nine", "ten", "page", "knight", "queen", "king"};
  static const char *major[] = {
    "fool", "magician", "high-priestess", "empress", "emperor",
    "hierophant", "lovers", "chariot", "strengts", "hermit",
...
int main(int argc, char **argv)
{
  ...
  pid_t child_pid = 0;
  int last_optind = 0;
  while ((option = getopt(argc, argv, "c:m:u:"))) { // shouldn't this check against -1 and not 0?
    switch (option) {
      case 'c':
...
/* check-linux-version */
  ...
  if (sscanf(host.release, "%u.%u", &major, &minor) != 2) {
    fprintf(stderr, "weirf release format: %s\n", host.release);
    goto cleanup;
  }
